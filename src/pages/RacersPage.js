...
export class RacersPage extends Component {
...
class Racer extends React.Component{
    ...
    save() {
        ...
        let racerToUpdate = this.state.item;
        
        racerToUpdate.name = document.getElementById('name'+this.state.item._id).value;
        racerToUpdate.surname = document.getElementById('surname'+this.state.item._id).value;
        let gender = document.getElementById('gender'+this.state.item._id);
        racerToUpdate.gender = gender.options[gender.selectedIndex].value;
        racerToUpdate.date_of_birth = document.getElementById('date_of_birth'+this.state.item._id).value;
        ...
        .then((response) => {
            this.props.onUpdate();
            if (response.ok) {
                alert('Saved changes for this racer.');
            } else {
                alert('Failed to save changes for this racer.');
            }
        })
    }
        ...
        return(
            ...
            <div className='rTableRow' key={this.state.item._id}>
                <div className='rTableCell'>
                    {
                        !this.state.editing ? this.state.item.name : (<input id={'name'+this.state.item._id} name='name' value={this.state.name} onChange={this.handleInputChange} type='text'/>)
                    }
                </div>
                <div className='rTableCell'>
                    {
                        !this.state.editing ? this.state.item.surname : (<input id={'surname'+this.state.item._id} name='surname' value={this.state.surname} onChange={this.handleInputChange} type='text'/>)
                    }
                </div>
                <div className='rTableCell'>
                    {
                        !this.state.editing ? birthdate : (<input id={'date_of_birth'+this.state.item._id} name='date_of_birth' value={birthdate} onChange={this.handleInputChange} type='date'/>)
                    }
                </div>
                <div className='rTableCell'>
                    {
                        !this.state.editing ? this.state.item.gender : (
                        <select id={'gender'+this.state.item._id} name='gender' value={this.state.gender} onChange={this.handleInputChange} >
